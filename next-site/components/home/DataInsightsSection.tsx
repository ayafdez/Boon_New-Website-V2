...
const competencies = [
  ...
  { name: 'Time Management',               before: 2.5, after: 3.5, delta: '+40%', barWidth: 58 },
  { name: 'Self Confidence',               before: 2.6, after: 3.4, delta: '+33%', barWidth: 48 },
  { name: 'Planning and Execution',        before: 2.7, after: 3.3, delta: '+25%', barWidth: 38 },
  { name: 'Emotional Intelligence',        before: 3.0, after: 3.7, delta: '+24%', barWidth: 34 },
];
...
export function DataInsightsSection() {
  return (
    <section
      className="section section-py relative overflow-hidden"
      style={{
        background: 'linear-gradient(135deg, #EEF1FF 0%, #F8F0FF 30%, #FFF5F0 70%, #FFF0F5 100%)',
      }}
    >
      {/* Soft gradient blobs */}
      <div className="absolute top-0 left-0 w-[50%] h-[60%] pointer-events-none" style={{ background: 'radial-gradient(ellipse at top left, rgba(70,111,246,0.12) 0%, transparent 65%)', filter: 'blur(60px)' }} />
      <div className="absolute bottom-0 right-0 w-[45%] h-[55%] pointer-events-none" style={{ background: 'radial-gradient(ellipse at bottom right, rgba(255,141,128,0.14) 0%, transparent 65%)', filter: 'blur(60px)' }} />

      <div className="container-xl relative z-10">
        <div className="grid lg:grid-cols-[1fr_1.15fr] gap-12 md:gap-16 lg:gap-20 items-center">
          ...
          <div className="relative">
            {/* Caption above card */}
            <p className="label-text text-[9px] text-gray-400 mb-4 text-right tracking-widest">
            ...
            >
              ...
              <div className="px-8 pt-8 pb-6 border-b border-gray-100/80">
                ...
                <p className="label-text text-[9px] text-gray-400 tracking-widest">
              ...
              <div className="px-8 py-6 space-y-0">
                {competencies.map((c, i) => (
                  <div
                    ...
                    style={{
                      gridTemplateColumns: '1fr 3rem 3rem 3.5rem 7rem',
                      borderBottom: i < competencies.length - 1 ? '1px solid rgba(0,0,0,0.05)' : 'none',
                    }}
                  >
                    <span className="text-sm font-medium text-gray-600 pr-4">{c.name}</span>
                    <span className="text-sm font-medium text-gray-400 text-right tabular-nums">{c.before.toFixed(1)}</span>
                    <span className="text-sm font-bold text-boon-blue text-right tabular-nums">{c.after.toFixed(1)}</span>
                    <span className="text-sm font-bold text-boon-blue text-right tabular-nums">{c.delta}</span>
